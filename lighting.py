...
def get_ambient(A,Ka):
    ...
    for x in range(len(A)):
        I_ambient.append(A[x]*Ka[x])
    return I_ambient
def get_diffuse_specular(P,L,V,N,Ka,Kd,Ks):
    I_diffuse=[]
    storage=[]
    for x in range(len(P)):
        storage.append(P[x]*Kd[x])
    ...
    for x in range(len(P)):
        I_specular.append(store[x]*memory)
    result=[]
    for x in range(len(I_diffuse)):
        result.append(I_diffuse[x]+I_specular[x])
    return result
...
def add_polygons(screen,buffer,matrix,A,light_list,V,Ka,Kd,Ks): #scanline conversion is written in scrip form within this code info is a list of ligts
    step=0
    while(step<len(matrix)):
        normal=surface_normal(matrix,step)
        Ia=get_ambient(A,Ka)
        Ids=get_diffuse_specular(light_list[0][1]['color'],light_list[0][1]['location'],V,normal,Ka,Kd,Ks)
        for x in light_list[1:]:
            store=get_diffuse_specular(x[1]['color'],x[1]['location'],V,normal,Ka,Kd,Ks)
            for x in range(len(store)):
                Ids[x]=Ids[x]+store[x]
        
        If=[]  #stands for I final
        for x in range(len(Ia)):
            If.append(int(Ia[x]+Ids[x]))
        for x in range(len(If)):
            if (If[x]>255):
                If[x]=255
            elif (If[x]<0):
                If[x]=0
...
def sphere(matrix,cx,cy,cz,radius,step=25):# n is 1+step
    rot=0
    t=0
    i=0
    j=0
